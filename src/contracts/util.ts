...
const envSchema = z.object({
  ...
  PUBLIC_ZK_HUNT_CONTRACT_ID: z.string().optional(),
...
const env: z.infer<typeof envSchema> = parsed.success
  ...
  : {
      ...
      PUBLIC_ZK_HUNT_CONTRACT_ID: undefined,
...
const getNetworkPassphraseForNetwork = (network: string): string => {
  switch (network) {
    case "LOCAL":
    case "NOIR":
 ...
 */
export const getZkHuntContractId = (skipStorage = false): string => {
  if (!skipStorage) {
    const stored = storage.getItem('zkHuntContractId', 'safe');
    if (stored) {
      return stored;
    }
  }
  const envContractId = env.PUBLIC_ZK_HUNT_CONTRACT_ID || import.meta.env.PUBLIC_ZK_HUNT_CONTRACT_ID;
